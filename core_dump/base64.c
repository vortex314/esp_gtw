 ...
 */
IROM static void cs_base64_emit_code(struct cs_base64_ctx *ctx, int v) {
  if (v < NUM_UPPERCASES) {
...
IROM static void cs_base64_emit_chunk(struct cs_base64_ctx *ctx) {
  int a, b, c;
...
IROM void cs_base64_init(struct cs_base64_ctx *ctx, cs_base64_putc_t b64_putc,
                    void *user_data) {
  ctx->chunk_size = 0;
...
IROM void cs_base64_update(struct cs_base64_ctx *ctx, const char *str, size_t len) {
...
IROM void cs_base64_finish(struct cs_base64_ctx *ctx) {
  if (ctx->chunk_size > 0) {
    int i;
...
IROM void cs_fprint_base64(FILE *f, const unsigned char *src, int src_len) {
  BASE64_ENCODE_BODY;
}
